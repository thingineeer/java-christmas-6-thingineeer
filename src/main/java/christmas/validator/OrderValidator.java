...
public class OrderValidator implements Validator {
    ...
    private void checkValidatedForm(final String input) { // 유효하지않은 입력 (기본 템플릿 입력이 아닌 경우) [요청 사항 존재]
        try {
    ...
    private void checkIfStringMenu(final String input) { // 메뉴(key) 부분이 String, 즉 문자열(한글,영어)이 아니면 예외처리 [요청 템플릿]
    ...
    private void checkQuantityRange(final String input) { // 수량이 각각 1이상 20이하 가 아니면 예외처리 [요청 사항 존재]
        try {
            Map<String, Integer> menu = Utils.makeStringToHashMap(input);
    ...
    private void checkIfMenuExists(final String input) { // 없는 메뉴일지 예외처리 ( ex) T본스테이크 ) [요청 사항 존재]
        Map<String, Integer> menu = Utils.makeStringToHashMap(input);
