...
- 방문 날짜 입력 기능
- 주문 할 메뉴와 개수 입력 기능 `(e.g. 해산물파스타-2,레드와인-1,초코케이크-1)`
- 방문 날짜에 해당하는 이벤트 혜택 미리보기 출력 기능
- 주문 메뉴 확인 기능
- 할인 전 총 주문 금액 출력 기능
  - 주문 메뉴 총합 관리 기능
- 증정메뉴 출력 기능
...
- 날짜입력
  - 숫자가 아닌 경우 예외
  - 숫자가 1이상 31이하가 아닌 경우 예외 [요청 사항 존재]
- 주문할 메뉴와 수량 입력
  - Map<String, Integer> 의 올바른 형식이 아닌 경우 예외 [요청 사항 존재]
  - 메뉴가 문자열(한글,영어)이 아니면 예외처리
  - 수량이 1이상 20이하 가 아니면 예외처리 [요청 사항 존재]
...
- [ ] 성공하는 케이스 뿐만 아니라 예외에 대한 케이스도 테스트 한다.
- [ ] 테스트 코드도 "코드"다.
